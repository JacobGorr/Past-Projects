                        ...
                        const Point2 &normalized_screen_pt, Point3 *sky_point)
{
    ...
    Matrix4 ivm = view_matrix.Inverse();
    Point3 eye = ivm.ColumnToPoint3(3);
    Point3 mouseIn3d = mingfx::GfxMath::ScreenToNearPlane(view_matrix,proj_matrix, normalized_screen_pt);
    Vector3 dir = mouseIn3d - eye;
    Ray eyeThroughMouse(eye, dir);// eye should be the origin
    float time;

    bool intersects = eyeThroughMouse.IntersectSphere( Point3(0,0,0), 1500, &time, &*sky_point);
    return intersects;
}
                       ...
                       const Mesh &stroke2d_mesh, const Color &stroke_color)
{
    ...
    for (int i=0; i < m.num_vertices(); i++) { //gets all vertices in 3d
      //printf("%d: %d\n",m.num_vertices(), i);
      Point2 pscreen = Point2(m.vertex(i)[0], m.vertex(i)[1]);
      Point3 newPoint;
      ScreenPtHitsSky(view_matrix, proj_matrix,pscreen,&newPoint);
